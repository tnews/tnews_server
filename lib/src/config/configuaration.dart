...
void _loadYamlFile(
    Map map, File yamlFile, Map<String, String> env, void warn(String msg)) async {
...
dynamic _applyEnv(var v, Map<String, String> env, void warn(String msg)) {
  if (v is String) {
    ...
    return v.keys.fold<Map>({}, (out, k) => out..[k] = _applyEnv(v[k], env ?? {}, warn));
  } else
    return v;
}
...
AngelConfigurer configuration(FileSystem fileSystem,
    {String directoryPath: './config', String overrideEnvironmentName, String envPath}) {
  return (Angel app) async {
    ...
    loadEnv('$directoryPath/$envPath', app);
    if (environmentName == 'development')
      loadEnv('.dev${envPath}', app);
    else
      loadEnv('$envPath', app);
